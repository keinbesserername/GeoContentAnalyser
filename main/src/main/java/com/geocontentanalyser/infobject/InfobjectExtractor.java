...
public class InfobjectExtractor extends Thread {
    ...
    // gets populated with InfobjectExtractorThread classes with each new doc being
    // parsed by SiteURLExtractor
    ...
    // used to write to an infobject.txt file consequtively, not concurrently, not
    // to corrupt it
        ...
        if (this.threads.size() < 2) {
            ...
            for (Semaphore semaphore : this.thread_semaphores) {
                if (semaphore.tryAcquire()) {
                    semaphore.release();
                    this.threads.get(thread_semaphores.indexOf(semaphore)).configure(doc, this.filename,
